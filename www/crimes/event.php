...
if (isset($_GET["event_id"]) && is_numeric($_GET["event_id"])) {
  ...
  //if (isset($_GET['ovd_id'])) $data["ovd_id"] = $_GET['ovd_id'];
...
<form class="current_object_form" id="<?php if (isset($event_id)) echo $event_id; ?>">
  ...
  <table border="0" rules="none" align="center" width="100%" class="object_view" object="<?= $object ?>">
    <tr>
      ...
      <td>
        Способ совершения:<span class="req">*</span>
        <?= isset($data["marking_id"]) ? marking($object, $data["marking_id"]) : marking($object); ?>
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td rowspan="4">
        ...
        <textarea name="story" style="margin: 0px; width: 400px; max-width: 400px; height: 120px;" req="true"><?= isset($data["story"]) ? $data["story"] : ""; ?></textarea>
      </td>
    </tr>
    <tr>
      <td>
    ...
    <tr>
      <td colspan="2" style="height: 75px;" valign="top">
        ...
        <table border="0" rules="none" width="100%" id="crime_table" <?= (isset($data["decision"]) && $data["decision"] == 1) ? 'style="display: none;"' : '' ?>>
          ...
          <tr>
            <td width="180px">
              <label>Изъяты образцы голоса <input type="checkbox" name="voice" value="1" <?php if (isset($data['voice'])) {if ($data['voice'] == 1) echo "checked";} ?>/></label>
            </td>
            <td>
