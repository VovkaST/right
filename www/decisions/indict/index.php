...
$query = '
  SELECT
    ovd.`id_ovd` as `id`, 
	ovd.`ovd`,
    IFNULL(`cnts`.`total`, 0) as `total`, 
	IFNULL(`debts`.`cnt_ovd`, 0) as `cnt_ovd`, IFNULL(`debts`.`cnt_sk`, 0) as `cnt_sk`,
  ...
  LEFT JOIN
    (
      SELECT DISTINCT
        d.d01_load as ovd,
        d.vd01_load,
        COUNT(DISTINCT IF(d.d33 != 1, d.rowid, NULL)) as `cnt_ovd`,
        COUNT(DISTINCT IF(d.d33 = 1, d.rowid, NULL))  as `cnt_sk`
      FROM
        `ic_for_load` as d
      LEFT JOIN
        `ic_f1_files` as f ON
...
$query = '
			...
			s_ovd.ovd,
			count(v.id) as total,
			ifnull(dl.cnt_ovd, 0) as cnt_ovd,
			ifnull(dl.cnt_sk, 0) as cnt_sk,
			COUNT( IF(MONTH(v.`create_date`) = 1, v.`id`, NULL)) as `jan`,
			COUNT( IF(MONTH(v.`create_date`) = 2, v.`id`, NULL)) as `feb`,
			COUNT( IF(MONTH(v.`create_date`) = 3, v.`id`, NULL)) as `mar`,
			COUNT( IF(MONTH(v.`create_date`) = 4, v.`id`, NULL)) as `apr`,
			COUNT( IF(MONTH(v.`create_date`) = 5, v.`id`, NULL)) as `may`,
			COUNT( IF(MONTH(v.`create_date`) = 6, v.`id`, NULL)) as `jun`,
			COUNT( IF(MONTH(v.`create_date`) = 7, v.`id`, NULL)) as `jul`,
			COUNT( IF(MONTH(v.`create_date`) = 8, v.`id`, NULL)) as `aug`,
			COUNT( IF(MONTH(v.`create_date`) = 9, v.`id`, NULL)) as `sep`,
			COUNT( IF(MONTH(v.`create_date`) = 10, v.`id`, NULL)) as `oct`,
			COUNT( IF(MONTH(v.`create_date`) = 11, v.`id`, NULL)) as `nov`,
			COUNT( IF(MONTH(v.`create_date`) = 12, v.`id`, NULL)) as `dec`

			from ( 
			select
			z.d01_load as id_ovd,
			z.vd01_load as ovd
			from ic_for_load as z 
			group by z.d01_load) as s_ovd left join 
					  (select
						o.ovd,
						o.id_ovd,
						ff.*
						from ic_f1_f11 as f1 
								left join spr_ovd as o on o.id_ovd = f1.d01_f10
									   join ic_f1_files as f on f1.id = f.f1
									   join l_files as ff on f.`file`=ff.id
							where ff.create_date between STR_TO_DATE("'.$year.'-01-01", "%Y-%m-%d") and STR_TO_DATE("'.$year.'-12-31", "%Y-%m-%d")
						 and ff.FilePath is not null
					  group by ff.id) as v on s_ovd.id_ovd= v.id_ovd
													left join 
														 (SELECT DISTINCT
														   d.d01_load as ovd,
														   d.vd01_load,
														...
														WHERE
														  f.`id` IS NULL and d.del = 0 -- and year(d.d25_f11) = '.$year.'
														GROUP BY
														  d.d01_load
													   ) as dl on dl.ovd = s_ovd.id_ovd
			group by s_ovd.id_ovd
			order by s_ovd.id_ovd
 ';
