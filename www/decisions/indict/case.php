...
try {
  ...
  $query = '
    ...
    select
  	  f3.N_GASPS
	from ic_f1_f11 as f1 left join ic_f3_f4 as f3 on f1.N_GASPS = f3.d8gasps_F3 
	  where f1.id = @cc AND f3.N_GASPS IS NOT NULL

	UNION 

	select
	f3.d8gasps_F3 
	from ic_f1_f11 as f1 left join ic_f3_f4 as f3 on f1.N_GASPS = f3.N_GASPS
	where f1.id = @cc AND f3.d8gasps_F3 IS NOT NULL';
  $_t = array();
  $result = $db->query($query);
  $date = $result -> fetch_All();
  foreach ($date as $zn) {
	        if (in_array('"'.$zn[0].'"', $_t) == false) { 
                     $_t[] = '"'.$zn[0].'"';
			}
  }
        ...
        TRIM(CONCAT(
          ...
          IF(f3.`d08_f3` IS NOT NULL,
            CONCAT(
			 ...
			 WHEN f3.d8gasps_F3 = f1.N_GASPS THEN
              
                CONCAT(
                  f3.`vd08_f3`, \' <a href="case.php?id=\', 
                  ifnull((SELECT f1.`id` FROM `ic_f1_f11` as f1 WHERE f3.N_GASPS = f1.N_GASPS AND f1.`d3o_f10` = 1 LIMIT 1),""), \'">\',
                  IFNULL(f3.`d3n_f3`, ""), " от ", IFNULL(f3.`d3g_f3`, ""), " г.")
				  
              ELSE
                CONCAT(
                  f3.`vd08_f3`, \' <a href="case.php?id=\',
                  ifnull((SELECT f1.`id` FROM `ic_f1_f11` as f1 WHERE f3.d8gasps_F3 = f1.N_GASPS AND f1.`d3o_f10` = 1 LIMIT 1),""), \'">\',
                  ifnull(f3.`d08n_f3`,""), " от ", DATE_FORMAT(f3.`d06_f3`, "%d.%m.%Y"))  
