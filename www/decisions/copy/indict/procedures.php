  ...
  if (!empty($_POST['method'])) {
    switch ($_POST['method']) {
      case 'fs': // first search - перичный поиск на index.php
        ...
        if (!empty($_POST['n']) or !empty($_POST['year'])) {
          // ******** поиск по у/д ******** //
          ...
          $query = '
            SELECT SQL_NO_CACHE
              f1.`id`,
              CONCAT(ifnull(f1.`vd3o_f10`,""), " № ", f1.`d3n_f10`, " от ", ifnull(DATE_FORMAT(f1.`d11_f10`, "%d.%m.%Y"),""), " ", ifnull(f1.`vd01_f10`,"")) as `string`,
              GROUP_CONCAT(
          ...
          $json['.search_result_block'] = '<div class="nothing_show">Пустой поисковый запрос</div>';
          ...
          if (isset($_POST['forms']['f5'])) {
            ...
            $queries['f5_1'] = '
              ...
              LEFT JOIN
                `ic_f1_f11` as f1 ON
                  f1.`d3n_f10` = f5.`d03n_f5` AND
                  YEAR(f1.`d11_f10`) = YEAR(f5.`d032_f5`) AND
                  f1.`d04_f10` = 1
              WHERE
                '.implode(' AND ', $p).'
            ';
          ...
          if (isset($_POST['forms']['f6'])) {
            ...
            $queries['f6'] = '
              ...
              JOIN
                ...
                LEFT JOIN
                  `ic_f1_f11` as f1 ON
                    f1.`d3n_f10` = f6.`d03n_f6` AND
                    YEAR(f1.`d11_f10`) = f6.`d03g_f6` AND
                    f1.`d04_f10` = 1
              WHERE
                '.implode(' AND ', $p).'
            ';
