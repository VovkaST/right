...
$wall_id = floor(abs($_GET["wall_id"]));
require_once(KERNEL.'connection.php');
$query = mysql_query('
  SELECT
    w.`id`, w.`payment_system`, w.`number`, 
    DATE_FORMAT(w.`reg_date`, "%d.%m.%Y") as reg_date,
    r.`id` as request_id,
...
if ($result['id'] < 1) header('Location: index.php'); // если кошелька не существует, выходим на index.php
?>
<!DOCTYPE html>
<html>
<head>
...
<form class="current_object_form" id="<?= $wall_id ?>">
  <center><span style="font-size: 1.2em;"><strong><?= current_object($object) ?></strong></span></center>
  <hr color="#C6C6C6" size="0px"/>
  <input type="hidden" name="data_form" value="form_e-wallet"/>
  <div class="object_id">
    <input type="hidden" name="id" value="<?= $wall_id ?>"/>
  </div>
    ...
    <tr>
      <td width="50%" align="right">
        <table border="0" rules="none" align="center" class="object_view" object="<?= $object ?>">
          <tr>
            <td align="right">Плат.система:</td>
            <td><?= ($result["payment_system"] != '') ? payment_systems($result["payment_system"]) : payment_systems(); ?></td>
          </tr>
          <tr>
            <td align="right">№:</td>
