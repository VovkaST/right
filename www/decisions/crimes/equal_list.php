...
SELECT
 r.`id` as `rel_id`,
                ...
                END as `type_id2`,
                CASE
                     WHEN  r.from_obj_type=2 THEN group_concat(ov.ovd, ' УД ', e.decision_number, ' от ', DATE_FORMAT(e.decision_date,'%d.%m.%Y')  separator '<br />')
                    -- WHEN r.from_obj_type=1 THEN group_concat(l.surname,' ', l.name,' ', l.fath_name separator ', ')
                END as `objects`,
                COUNT(
                  CASE
                    WHEN o.`id` = r.`from_obj_type` THEN r.`to_obj`
              ...
              JOIN
                `l_relatives` as r ON
                  ...
                  END AND
                  r.`ais` = 1
            left join o_event as e on r.from_obj_type=2 and e.id=r.from_obj
            -- left join o_lico as l on r.from_obj_type=1 and l.id=r.from_obj
            left join spr_ovd as ov on e.ovd_id=ov.id_ovd
              WHERE
                o.`id` <> 2
              GROUP BY
                `obj_id1`, `type_id1`, `type_id2`
...
switch ($object) {
  ...
  case 5:
    $query = mysql_query('
      SELECT
        `equals`.`obj_id1` as `id`, d.`IMEI`, d.`model`, `equals`.`objects`
      FROM
        ('.$temp_tbl.') as `equals`
      JOIN
