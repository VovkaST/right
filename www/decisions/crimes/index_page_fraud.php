...
$query = '
  ...
  WHERE
    ovd.`visuality` = 1 AND
    ovd.`id_ovd` NOT IN(5,6,7,10,60,61,62,63,64,65,66,135,103, 136, 137,68)
';
$result = mysql_query($query);
...
$query = '
  ...
  WHERE
    e.`ais` = 1 AND
    e.`create_date` BETWEEN STR_TO_DATE("01.01.'.$year.'", "%d.%m.%Y") AND STR_TO_DATE("31.12.'.$year.'", "%d.%m.%Y")
  GROUP BY
    e.`ovd_id`
';
$result = mysql_query($query) or die(mysql_error().'<pre>'.$query.'</pre>');
...
//------Долги по IMEI-----------//
  $query = "
          select ovd, count(id) as `debt` from 
                (select
                     e.ovd_id as ovd, 
                     e.id,
                     e.`story`,
                ...
                and e.ovd_id not in (5,6,7,10,60,61,62,63,64,65,66,135,103, 136, 137,68)
          ...
          group by yy.ovd
        ";  
          /*select ovd, count(idd) as `debt` from (select e.id as `idd`, e.ovd_id as `ovd`, e.decision_number as `ud`, e.decision_date as `date` 
          from o_event as e
          left join l_relatives as r
          ...
          and substring(t.number, 1, 1) = '9'
          group by e.id) as ttt
          group by `ovd`*/
    
$result = mysql_query($query) or die(mysql_error().'<pre>'.$query.'</pre>');
while ($row = mysql_fetch_array($result)) {
  $comb[$row['ovd']]['imei'] = $row['debt'];
}
mysql_free_result($result);
//------!Долги по IMEI-----------//
...
$per_date = mysql_query("
		  			    select
					 	       if(DAYOFWEEK(now()) = 7 or DAYOFWEEK(now()) = 1, date_format(date_sub(now(), interval 4 day), '%d.%m.%y'), date_format(date_sub(now(), interval 2 day), '%d.%m.%y')) as dt_s,
							   date_format(now(), '%d.%m.%y') as dt_t
						") or die(mysql_error());
$p_date = mysql_fetch_assoc($per_date);


$q_objects = mysql_query('
  SELECT
    o.`id`,
    -- if(o.`id`=14, concat("Cовпадения с '.$p_date['dt_s'].'", " по " , "'.$p_date['dt_t'].'"),o.`object`) as object,
    o.`object` as object,
    CASE
...
") or die(mysql_error());
$p_days_tel = mysql_fetch_assoc($per_days);

        
 $per_days = mysql_query("select
					distinct
					count(k.id) as count
					from (
						select
						...
						t.IMEI,
						count(svt.to_obj)
						from o_mobile_device as t 
						join l_relatives as svt 
						on t.id = svt.to_obj and svt.to_obj_type=5 and svt.from_obj_type = 2
						join spr_relatives as _svt 
						on _svt.to_obj = 5 and _svt.id = svt.`type`
						join o_event as e 
						on e.id = svt.from_obj and svt.from_obj_type = 2
						where svt.ais = 1 and svt.create_date between if(DAYOFWEEK(now()) = 7 or DAYOFWEEK(now()) = 1, date_format(date_sub(now(), interval 4 day), '%Y-%m-%d'), date_format(date_sub(now(), interval 2 day), '%Y-%m-%d')) and date_format(now(), '%Y-%m-%d')
							  and e.kusp_num is not null
						group by svt.to_obj
						having count(svt.to_obj)>1
						order by t.IMEI ) as dd 
						
						left join l_relatives as sv 
						on sv.to_obj = dd.id_t 
						and sv.to_obj_type = 5 
						and sv.ais = 1 
						and sv.from_obj_type = 2
					   join o_event as k on k.id = sv.from_obj
						join spr_ovd as ovd on k.ovd_id=ovd.id_ovd
				order by dd.IMEI, k.kusp_date desc, k.decision_date desc
") or die(mysql_error());
$p_days_imei = mysql_fetch_assoc($per_days);
?>
...
<table rules="all" border="1" cellpadding="3" rules="all" class="total_indicators result_table">
  <tr class="table_head">
    ...
    <th width="100px">Нет<br/>IMEI</th>
 <!--   <th width="65px">Не заполн.<br/>реквизит<br/>ф.1.0</th> -->
  </tr>
  <?php

    $total = $count = $disclosed = $debt = $imei = $debt_ic = $sum_depts = 0;
    ...
    <tr>
      ...
      <td align="center">
      <?php if (!isset($data["imei"]) or $data["imei"] == 0) : ?>
        0
      <?php else : ?>
        <a href="<?= 'imei_debt_list.php?ovd_id='.$ovd ?>&year=<?= $year ?>"><?= $data["imei"] ?></a><?php $imei += $data["imei"]; ?>
      <?php endif; ?>
      </td>
 <!--
      <td align="center">
...
<table rules="all" border="1" cellpadding="3" rules="all" class="objects_indicators result_table">
      ...
      <?php endwhile; ?>
      <tr>
        <td>Совпадения с <?=$p_date['dt_s']?> по <?=$p_date['dt_t']?><br />(Телефоны)</td>
        <td></td>
         <td align="center"><a href="objects_list.php?object=144"><?= $p_days_tel['count'] ?></a></td>
      </tr>
      <tr>
        <td>Совпадения с <?=$p_date['dt_s']?> по <?=$p_date['dt_t']?><br />(IMEI)</td>
        <td></td>
         <td align="center"><a href="objects_list.php?object=145"><?= $p_days_imei['count'] ?></a></td>
      </tr>
      <tr style="background: #F4F4F4;">
        <th>Итого</th>
