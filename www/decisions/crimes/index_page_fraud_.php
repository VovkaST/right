...
/*
$query = "
select mo.kod_ovd, count(mo.id) from ic_dist_crimes as mo
left join 
(
SELECT DISTINCT
    e.`id`, e.`kusp_num`,
  ...
  LEFT JOIN
    `l_object_marking` as om ON
      ...
      om.`obj_type` = 2
  WHERE
     YEAR(e.`decision_date`) = 2018 and e.`ais` = 1
  GROUP BY
   e.`id`
  ORDER BY
    e.`kusp_date`) as ours
  ...
  <?php
    ...
    <tr>
      ...
      <?php if (!isset($data["debt"]) or $data["debt"] == 0) : ?>
        0
      <?php else : ?>
        <a href="<?= 'events_dept.php?ovd_id='.$ovd ?>&type=1"><?= $data["debt"] ?></a><?php $debt += $data["debt"]; ?>
      <?php endif; ?>
      </td>
 <!--     
      <td align="center">
