  ...
  <form type="json" class="main_form">
    ...
    <div class="input_row">
      <div class="field_box">
        ...
        <?php if (is_null($data->get_ovd())) : ?>
          <?= my_select('ovd', 'spr_ovd') ?>
        <?php else : ?>
          <div class="input_field_block" style="width: 300px;">
            <input type="text" autocomplete="off" value="<?= $data->get_ovd_string() ?>" disabled/>
          </div>
        <?php endif; ?>
      </div>
      <div class="field_box">
        <span class="field_name">Дата:</span>
        <?php echo (is_null($data->get_date())) ? my_date_field('date') : my_date_field('date', date('d.m.Y', strtotime($data->get_date())), null, 'disabled') ?>
      </div>
      
      <div class="field_box">
        <span class="field_name">№:</span>
        <div class="input_field_block">
          <input type="text" value="<?= (!is_null($data->get_number())) ? $data->get_number() : 'Без номера' ?>" disabled/>
        </div>
      </div>
    </div>
