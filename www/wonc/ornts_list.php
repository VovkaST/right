...
if (isset($_GET['recalled'])) {
  ...
  $where[] = 'o.`recall` IS NULL AND dh.`dec_code` IN (2,12,91,25,27)';
  $variant = ' (Не снятые)';
}
...
$query = '
  SELECT
    ...
    f.`link`, IF(fc.`id` IS NULL, "false", "true") as `indexed`
  FROM
    `l_orientations` as o
  LEFT JOIN
    `spr_uk` as uk ON
...
<?php if ($result = mysql_query($query)) : ?>
  ...
  ?>
    <div class="result_row">
      <div class="result_cell number"><a href="ornt_view.php?id=<?= $row['id'] ?>"><?= (($row['wonumber']) ? 'б/н' : $row['number']) ?></a></div>
