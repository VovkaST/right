...
function send_simple_fields($field, $par, $handling = 0) {
  if ($handling) {
  ...
  if (in_array($file_type, $file_type_array)) {
    if ($file_size != "0" && $file_size < "2097152") {
      if (is_uploaded_file($file_tmp_name)) {
        if (copy($file_tmp_name, $_SESSION['dir_session'].iconv('UTF-8', 'Windows-1251', $file_name))) {
          $_SESSION["refusal"]["uploaded_file"] = $file_name;
          if (strlen($file_name) > 33) {
            $file_name = mb_strcut($file_name, 0, 50, 'UTF-8').'...'.$file_type;
          }
          echo '<script type="text/javascript">window.parent.toggle_file_input()</script>';
          echo '<script type="text/javascript">window.parent.$(".uploaded_file_form").html(\'<form class="uploaded_file" method="POST">Файл: <b>'.$file_name.'</b><label class="del_str"><input type="image" src="#" class="del" /><span class="del_str_but"><strong>&times;</strong>Удалить</span></label><input type="hidden" name="file_delete" value="true"/></form>\')</script>';
        } else {
          echo '<script type="text/javascript">window.parent.$(".uploaded_file_form").html(\'<div class="error"><p>Ошибка копирования файла!</p></div>\')</script>';
        }
      } else {
        echo '<script type="text/javascript">window.parent.$(".uploaded_file_form").html(\'<div class="error"><p>Ошибка загрузки файла!</p></div>\')</script>';
      }
    } else {
      echo '<script type="text/javascript">window.parent.$(".uploaded_file_form").html(\'<div class="error"><p>Файл имеет 0 размер или больше 2 мб!</p></div>\')</script>';
    }
  } else {
    echo '<script type="text/javascript">window.parent.$(".uploaded_file_form").html(\'<div class="error"><p>Недопустимый тип файла!</p></div>\')</script>';
  }
}

// удаление загруженного файла
if (isset($_POST['file_delete'])) {
  if (unlink($_SESSION['dir_session'].iconv('UTF-8', 'Windows-1251', $_SESSION["refusal"]["uploaded_file"]))) {
    echo '<div class="success">Файл удален!</div>';
...
if (isset($_POST["ovd_otkaz"])) {
  send_simple_fields($_POST["ovd_otkaz"], 'ovd');
}

//посылка статуса
if (isset($_POST["status"])) {
  send_simple_fields($_POST["status"], 'status');
}

//посылка службы
if (isset($_POST["slujba"])) {
  send_simple_fields($_POST["slujba"], 'slujba');
}

//посылка фамилии
if (isset($_POST["surname"])) {
  send_simple_fields($_POST["surname"], 'surname', 1);
}

//посылка имени
if (isset($_POST["name"])) {
  send_simple_fields($_POST["name"], 'name', 1);
}

//посылка отчества
if (isset($_POST["father_name"])) {
  send_simple_fields($_POST["father_name"], 'father_name', 1);
}

//посылка УПК
if (isset($_POST["upk"])) {
  send_simple_fields($_POST["upk"], 'upk');
}
...
if (!empty($_POST['debt_id'])) {
  ...
  send_simple_fields($result["id_ovd"], 'ovd');
  send_simple_fields($result["id_slujba"], 'slujba');
  send_simple_fields($result["emp_s"], 'surname', 1);
  send_simple_fields($result["emp_n"], 'name', 1);
  send_simple_fields($result["emp_fn"], 'father_name', 1);
  send_date($result["dec_date"]);
  if (!empty($result["article_proc_par"])) {
    $proc = substr($result["article_proc_par"], 0, 1);
    if ($proc > 0 && $proc <= 6) {
      send_simple_fields($proc, 'upk');
    }
  }
