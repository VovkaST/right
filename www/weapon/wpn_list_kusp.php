...
if(isset($_GET['year'])) array_push($where, 'year(wa.reg_date) = '.$_GET['year']);
array_push($where, '(wa.kusp is not NULL or wa.crime_case is not NULL)');
...
$query = '
  SELECT DISTINCT
    concat(\'КУСП \',lk.kusp, \' от \',date_format(lk.`date`, \'%d.%m.%Y\')) as `kusp`,
    concat(\'УД \',cc.crime_case_number, \' от \', date_format(cc.crim_case_date, \'%d.%m.%Y\')) as `crime_case`,
  ...
  <div class="result_row">
    ...
    <div class="result_cell left-align">
      <ul>
        ...
        <?php if (!empty($row->kusp)) echo '<li> '.$row->kusp.'</li>' ?>
        <?php if (!empty($row->crime_case)) echo '<li> '.$row->crime_case.' </li>' ?>
      </ul>
