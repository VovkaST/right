  ...
  <div class="left_box">
    <div id="yearList">
      <?php foreach ($yearList as $value) :?>
        <a class="yearListStr" <?php if ($value == $year) echo 'id="resultYearBrowse"' ?> href="<?= $_SERVER["PHP_SELF"].'?volume=kusp&year='.$value ?>"><?= $value ?></a>
      <?php endforeach; ?>
    </div>
  ...
  <?php while ($row = $result->fetch_object()) : ?>
    <tr>
      ...
      <td align="center" title="<?= $row->ovd ?>, Всего зарегистрировано квитанций">
        <?= (empty($row->accounts)) ? 0 : '<a href="acc_list_kusp.php?ovd='.$row->id.'&year='.$year.'">'.$row->accounts.'</a>' ?>
        <?php $total[0] += $row->accounts ?>
      </td>
      <td align="center" title="<?= $row->ovd ?>, Всего единиц оружия">
        <?= (empty($row->weapons)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&year='.$year.'">'.$row->weapons.'</a>' ?>
        <?php $total[1] += $row->weapons ?>
      </td>
      
      <td align="center" title="<?= $row->ovd ?>, Всего единиц оружия (Огнестрельное оружие)">
        <?= (empty($row->firearms)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=fa&year='.$year.'">'.$row->firearms.'</a>' ?>
        <?php $total[2] += $row->firearms ?>
      </td>
      <td align="center" title="<?= $row->ovd ?>, Всего единиц оружия (Боеприпасы)">
        <?= (empty($row->ammunition)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=amm&year='.$year.'">'.$row->ammunition.'</a>' ?>
        <?php $total[3] += $row->ammunition ?>
      </td>
      <td align="center" title="<?= $row->ovd ?>, Всего единиц оружия (Взрывные устройства (вещества))">
        <?= (empty($row->explosives)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=exp&year='.$year.'">'.$row->explosives.'</a>' ?>
        <?php $total[4] += $row->explosives ?>
      </td>
      <td align="center" title="<?= $row->ovd ?>, Всего единиц оружия (Холодное оружие)">
        <?= (empty($row->steelarms)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=sa&year='.$year.'">'.$row->steelarms.'</a>' ?>
        <?php $total[5] += $row->steelarms ?>
      </td>
      
      
      <td align="center" title="<?= $row->ovd ?>, Место хранения &ndash; в ОВД (Всего)">
        <?= (empty($row->in_ovd)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InOVD&year='.$year.'">'.$row->in_ovd.'</a>' ?>
        <?php $total[6] += $row->in_ovd ?>
      </td>
      <td align="center" title="<?= $row->ovd ?>, Огнестрельное оружие, Место хранения &ndash; в ОВД">
        <?= (empty($row->in_ovd_firearms)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InOVD&wtype=1&year='.$year.'">'.$row->in_ovd_firearms.'</a>' ?>
        <?php $total[7] += $row->in_ovd_firearms ?>
      </td>
      <td align="center" title="<?= $row->ovd ?>, Боеприпасы, Место хранения &ndash; в ОВД">
        <?= (empty($row->in_ovd_ammunition)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InOVD&wtype=2&year='.$year.'">'.$row->in_ovd_ammunition.'</a>' ?>
        <?php $total[8] += $row->in_ovd_ammunition ?>
      </td>
      <td align="center" title="<?= $row->ovd ?>, Взрывные устройства (вещества), Место хранения &ndash; в ОВД">
        <?= (empty($row->in_ovd_explosives)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InOVD&wtype=3&year='.$year.'">'.$row->in_ovd_explosives.'</a>' ?>
        <?php $total[9] += $row->in_ovd_explosives ?>
      </td>
      <td align="center" title="<?= $row->ovd ?>, Холодное оружие, Место хранения &ndash; в ОВД">
        <?= (empty($row->in_ovd_steelarms)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InOVD&wtype=4&year='.$year.'">'.$row->in_ovd_steelarms.'</a>' ?>
        <?php $total[10] += $row->in_ovd_steelarms ?>
      </td>
      
      <td align="center" title="<?= $row->ovd ?>, Место хранения &ndash; в ОВД более 60 суток (Всего)">
        <?= (empty($row->in_ovd_60)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InOVD60&year='.$year.'">'.$row->in_ovd_60.'</a>' ?>
        <?php $total[11] += $row->in_ovd_60 ?>
      </td>
      <td align="center" title="<?= $row->ovd ?>, Огнестрельное оружие, Место хранения &ndash; в ОВД более 60 суток">
        <?= (empty($row->in_ovd_60_firearms)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InOVD&wtype=1&year='.$year.'">'.$row->in_ovd_60_firearms.'</a>' ?>
        <?php $total[12] += $row->in_ovd_60_firearms ?>
      </td>
      <td align="center" title="<?= $row->ovd ?>, Боеприпасы, Место хранения &ndash; в ОВД более 60 суток">
        <?= (empty($row->in_ovd_60_ammunition)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InOVD&wtype=2&year='.$year.'">'.$row->in_ovd_60_ammunition.'</a>' ?>
      ...
      <td align="center" title="<?= $row->ovd ?>, Взрывные устройства (вещества), Место хранения &ndash; в ОВД более 60 суток">
        <?= (empty($row->in_ovd_60_explosives)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InOVD&wtype=3&year='.$year.'">'.$row->in_ovd_60_explosives.'</a>' ?>
      ...
      <td align="center" title="<?= $row->ovd ?>, Холодное оружие, Место хранения &ndash; в ОВД более 60 суток">
        <?= (empty($row->in_ovd_60_steelarms)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InOVD&wtype=4&year='.$year.'">'.$row->in_ovd_60_steelarms.'</a>' ?>
        <?php $total[15] += $row->in_ovd_60_steelarms ?>
      </td>
      
      <td align="center" title="<?= $row->ovd ?>, Место хранения &ndash; на складе УМВД (Всего)">
        <?= (empty($row->in_storage)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InStor&year='.$year.'">'.$row->in_storage.'</a>' ?>
        <?php $total[16] += $row->in_storage ?>
      </td>
      <td align="center" title="<?= $row->ovd ?>, Огнестрельное оружие, Место хранения &ndash; на складе УМВД">
        <?= (empty($row->in_storage_firearms)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InStor&wtype=1&year='.$year.'">'.$row->in_storage_firearms.'</a>' ?>
      ...
      <td align="center" title="<?= $row->ovd ?>, Боеприпасы, Огнестрельное оружие, Место хранения &ndash; на складе УМВД">
        <?= (empty($row->in_storage_ammunition)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InStor&wtype=2&year='.$year.'">'.$row->in_storage_ammunition.'</a>' ?>
      ...
      <td align="center" title="<?= $row->ovd ?>, Взрывные устройства (вещества), Место хранения &ndash; на складе УМВД">
        <?= (empty($row->in_storage_explosives)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InStor&wtype=3&year='.$year.'">'.$row->in_storage_explosives.'</a>' ?>
        <?php $total[19] += $row->in_storage_explosives ?>
      </td>
      
      <td align="center" title="<?= $row->ovd ?>, Место хранения &ndash; на складе УМВД более 1 года">
        <?= (empty($row->in_storage_1y)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InStor1y&year='.$year.'">'.$row->in_storage_1y.'</a>' ?>
        <?php $total[20] += $row->in_storage_1y ?>
      </td>
      <td align="center" title="<?= $row->ovd ?>, Боеприпасы, Место хранения &ndash; на складе УМВД более 1 года">
        <?= (empty($row->in_storage_1y_firearms)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InStor1y&wtype=1&year='.$year.'">'.$row->in_storage_1y_firearms.'</a>' ?>
      ...
      <td align="center" title="<?= $row->ovd ?>, Взрывные устройства (вещества), Место хранения &ndash; на складе УМВД более 1 года">
        <?= (empty($row->in_storage_1y_ammunition)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InStor1y&wtype=2&year='.$year.'">'.$row->in_storage_1y_ammunition.'</a>' ?>
        <?php $total[22] += $row->in_storage_1y_ammunition ?>
      </td>
      <td align="center" title="<?= $row->ovd ?>, Место хранения &ndash; на складе УМВД более 1 года">
        <?= (empty($row->in_storage_1y_explosives)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InStor1y&wtype=3&year='.$year.'">'.$row->in_storage_1y_explosives.'</a>' ?>
        <?php $total[23] += $row->in_storage_1y_explosives ?>
      </td>
      <td align="center" title="<?= $row->ovd ?>, Место хранения &ndash; на складе УМВД более 1 года">
        <?= (empty($row->in_storage_1y_steelarms)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=InStor1y&wtype=4&year='.$year.'">'.$row->in_storage_1y_steelarms.'</a>' ?>
        <?php $total[24] += $row->in_storage_1y_steelarms ?>
      </td>
      
      <td align="center" title="<?= $row->ovd ?>, Утилизировано">
        <?= (empty($row->util)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=Util&year='.$year.'">'.$row->util.'</a>' ?>
        <?php $total[25] += $row->util ?>
      </td>
      <td align="center" title="<?= $row->ovd ?>, Возвращено владельцу, Затребовано сотрудником">
        <?= (empty($row->issued)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=issued&year='.$year.'">'.$row->issued.'</a>' ?>
        <?php $total[26] += $row->issued ?>
      </td>
      <td align="center" title="<?= $row->ovd ?>, Направлено исковое заявление в суд">
        <?= (empty($row->lawsuit)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=lowsuit&year='.$year.'">'.$row->lawsuit.'</a>' ?>
        <?php $total[27] += $row->lawsuit ?>
      </td>
      <td align="center" title="<?= $row->ovd ?>, Направлено извещение собственнику">
        <?= (empty($row->notice)) ? 0 : '<a href="wpn_list_kusp.php?ovd='.$row->id.'&clause=notice&year='.$year.'">'.$row->notice.'</a>' ?>
